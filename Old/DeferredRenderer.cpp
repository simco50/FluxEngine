...
void DeferredRenderer::CreateGBuffer()
{
	...
	m_pGBuffer[DIFFUSE] = make_unique<RenderTarget>(Renderer::Instance().GetDevice(), Renderer::Instance().GetDeviceContext());
	m_pGBuffer[DIFFUSE]->Create(desc);

	desc.ColorFormat = DXGI_FORMAT_R32G32B32A32_FLOAT;
	m_pGBuffer[NORMAL] = make_unique<RenderTarget>(Renderer::Instance().GetDevice(), Renderer::Instance().GetDeviceContext());
...
void DeferredRenderer::End()
{
	...
	Renderer::Instance().GetDeviceContext()->OMSetRenderTargets(GBUFFER_SIZE, pRTV, nullptr);

	//Set to the default render target
	ID3D11RenderTargetView* pDefaultRTV = Renderer::Instance().GetDefaultRenderTarget()->GetRenderTargetView();
	Renderer::Instance().GetDeviceContext()->OMSetRenderTargets(1, &pDefaultRTV, nullptr);
	...
	m_pMaterial->SetDepthSRV(Renderer::Instance().GetDefaultRenderTarget()->GetDepthSRV());
	...
	Renderer::Instance().GetDeviceContext()->PSSetShaderResources(0, GBUFFER_SIZE + 2, pSRV);
}
...
void DeferredRenderer::ClearRenderTargets()
{
	...
	for (size_t i = 0; i < GBUFFER_SIZE; i++)
	{
		...
		Renderer::Instance().GetDeviceContext()->ClearRenderTargetView(pRTV, reinterpret_cast<float*>(&clearColor));
	}
	Renderer::Instance().GetDeviceContext()->ClearDepthStencilView(Renderer::Instance().GetDefaultRenderTarget()->GetDepthStencilView(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);

	ID3D11RenderTargetView* pRTV = m_pGBuffer[0]->GetRenderTargetView();
	Renderer::Instance().GetDeviceContext()->ClearRenderTargetView(pRTV, reinterpret_cast<float*>(&m_pGameContext->Engine->GameSettings.ClearColor));
}
...
void DeferredRenderer::SetRenderTargets()
{
	...
	Renderer::Instance().GetDeviceContext()->OMSetRenderTargets(GBUFFER_SIZE, pRTVs.data(), Renderer::Instance().GetDefaultRenderTarget()->GetDepthStencilView());
}
