...
{
	Renderer::Instance().GetDeviceContext()->ClearDepthStencilView(m_pRenderTarget->GetDepthStencilView(), D3D11_CLEAR_STENCIL | D3D11_CLEAR_DEPTH, 1.0f, 0);

	ID3D11RenderTargetView* pRTV = m_pRenderTarget->GetRenderTargetView();
	Renderer::Instance().GetDeviceContext()->OMSetRenderTargets(1, &pRTV, m_pRenderTarget->GetDepthStencilView());

	Renderer::Instance().GetDeviceContext()->RSSetViewports(1, &m_Viewport);
...
{
	Renderer::Instance().GetDeviceContext()->RSSetViewports(1, &m_pGameContext->Scene->Camera->GetViewport());
	ID3D11RenderTargetView* pRTV = Renderer::Instance().GetDefaultRenderTarget()->GetRenderTargetView();
	Renderer::Instance().GetDeviceContext()->OMSetRenderTargets(1, &pRTV, Renderer::Instance().GetDefaultRenderTarget()->GetDepthStencilView());
}
...
void ShadowMapper::SetResolution(const int width, const int height)
{
	...
	m_pRenderTarget = make_unique<RenderTarget>(Renderer::Instance().GetDevice(), Renderer::Instance().GetDeviceContext());
