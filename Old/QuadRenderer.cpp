...
{
	Renderer::Instance().GetDeviceContext()->IASetInputLayout(pMaterial->GetInputLayout());

	UINT strides = sizeof(VertexPosTex);
	UINT offset = 0;
	Renderer::Instance().GetDeviceContext()->IASetVertexBuffers(0, 1, m_pVertexBuffer.GetAddressOf(), &strides, &offset);

	Renderer::Instance().GetDeviceContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);

	D3DX11_TECHNIQUE_DESC techDesc;
	pMaterial->GetTechnique()->GetDesc(&techDesc);
	for (size_t i = 0; i < techDesc.Passes; i++)
	{
		pMaterial->GetTechnique()->GetPassByIndex(0)->Apply(0, Renderer::Instance().GetDeviceContext());
		Renderer::Instance().GetDeviceContext()->Draw(4, 0);
	}
}
...
void QuadRenderer::CreateVertexBuffer()
{
	...
	HR(Renderer::Instance().GetDevice()->CreateBuffer(&bd, &initData, m_pVertexBuffer.GetAddressOf()));
}
