...
bool ShaderVariation::Create(ID3D11Device* pDevice)
{
	...
	case ShaderType::VertexShader:
		HR(pDevice->CreateVertexShader(m_ShaderByteCode.c_str(), m_ShaderByteCode.size(), 0, (ID3D11VertexShader**)&m_pShaderObject))
			break;
	case ShaderType::PixelShader:
		HR(pDevice->CreatePixelShader(m_ShaderByteCode.c_str(), m_ShaderByteCode.size(), 0, (ID3D11PixelShader**)&m_pShaderObject))
			break;
	default:
		break;
	}
...
bool ShaderVariation::Compile()
{
	...
	HRESULT hr = D3DCompile(source.c_str(), source.size(), "", macros.data(), 0, entryPoint, profile, flags, 0, &shaderCode, &errorMsgs);
	if (hr != S_OK)
	{
		string errorMessage((char*)errorMsgs->GetBufferPointer(), (char*)errorMsgs->GetBufferPointer() + errorMsgs->GetBufferSize());
		Console::Log(errorMessage, LogType::ERROR);
		return false;
	}
