...
RenderTarget::RenderTarget(Context* pContext, Graphics* pGraphics) :
	Object(pContext),
...
bool RenderTarget::Create(const RenderTargetDesc& RenderTargetDesc)
{
	...
	m_pRenderTexture = make_unique<Texture>(m_pContext, m_pGraphics);
	...
	m_pDepthTexture = make_unique<Texture>(m_pContext, m_pGraphics);
