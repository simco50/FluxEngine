...
void Texture::UpdateParameters()
{
	...
	desc.ComparisonFunc = D3D11Helpers::D3D11ComparisonFunction(CompareMode::LESSEQUAL);
	desc.Filter = D3D11Helpers::D3D11Filter(TextureFilter::MIN_MAG_MIP_LINEAR);
...
unsigned int Texture::GetDSVFormat(const unsigned int format)
{
	...
	default:
		checkf(false, "[Texture::GetDSVFormat] Texture format not supported");
		return 0;
	}
}
...
unsigned int Texture::TextureFormatFromCompressionFormat(const ImageFormat& format, bool sRgb)
{
	...
	default:
		checkf(false, "[TextureFormatFromCompressionFormat] Texture format not supported");
		return 0;
	}
}
