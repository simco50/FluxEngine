...
void StructuredBuffer::Create(const int elementCount, const int elementStride, bool dynamic /*= false*/)
{
	...
	viewDesc.Buffer.ElementWidth = elementStride;
	viewDesc.Buffer.FirstElement = 0;
	viewDesc.Buffer.NumElements = elementCount;
	viewDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;

	HR(m_pGraphics->GetImpl()->GetDevice()->CreateShaderResourceView((ID3D11Buffer*)m_pBuffer, &viewDesc, (ID3D11ShaderResourceView**)&m_pShaderResourceView));
}
...
void* StructuredBuffer::Map(bool discard)
{
	if (m_Mapped == false)
	{
		...
		HR(m_pGraphics->GetImpl()->GetDeviceContext()->Map((ID3D11Buffer*)m_pBuffer, 0, discard ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_WRITE, 0, &data));
