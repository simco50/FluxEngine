...
HRESULT Renderer::Initialize(const RendererDesc& desc)
{
	m_RendererDesc = desc;
...
void Renderer::RenderImmediate(const RenderItem& item)
{
	RenderSingle(item);
}
...
void Renderer::SetRenderTargets(const vector<RenderTarget*>& renderTargets)
{
...
void Renderer::OnResize()
{
	...
	desc.MSAA = m_RendererDesc.Msaa;
	desc.MsaaQuality = m_RendererDesc.MsaaQuality;
	desc.DepthSRV = true;
	desc.ClearColor = m_RendererDesc.ClearColor;
...
void Renderer::SetInputLayout(ID3D11InputLayout* pInputLayout)
{
...
void Renderer::SetVertexBuffer(const int startSlot, ID3D11Buffer* pVertexBuffer, UINT* pStrides, UINT* pOffset)
{
	SetVertexBuffers(startSlot, { pVertexBuffer }, pStrides, pOffset);
}

void Renderer::SetVertexBuffers(const int startSlot, const vector<ID3D11Buffer*>& pVertexBuffers, UINT* pStrides, UINT* pOffset)
{
	m_pDeviceContext->IASetVertexBuffers(startSlot, pVertexBuffers.size(), pVertexBuffers.data(), pStrides, pOffset);
}

void Renderer::SetIndexBuffer(ID3D11Buffer* pIndexBuffer, const DXGI_FORMAT format /*= DXGI_FORMAT_R32_UINT*/, const UINT offset /*= 0*/)
{
