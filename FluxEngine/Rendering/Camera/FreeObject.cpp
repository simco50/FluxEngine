...
FreeObject::FreeObject(Context* pContext) :
	SceneNode(pContext)
{
...
void FreeObject::OnSceneSet(Scene* pScene)
{
	...
	m_UpdateHandle = pScene->OnSceneUpdate().AddRaw(this, &FreeObject::Update);
}

void FreeObject::OnSceneRemoved()
{
	m_pScene->OnSceneUpdate().Remove(m_UpdateHandle);
	SceneNode::OnSceneRemoved();
}

void FreeObject::Update()
{
