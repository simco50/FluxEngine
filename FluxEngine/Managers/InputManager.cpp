...
bool InputManager::IsKeyboardKeyDown(int key) const
{
...
bool InputManager::IsGamepadButtonDown(WORD button, GamepadIndex playerIndex) const
{
	return (m_CurrGamepadState[playerIndex].Gamepad.wButtons&button) != 0 ? true : false;
}

XMFLOAT2 InputManager::GetThumbstickPosition(bool leftThumbstick, GamepadIndex playerIndex) const
{
	XMFLOAT2 pos;
	if(leftThumbstick)
	{
...
void InputManager::SetVibration(float leftVibration, float rightVibration, GamepadIndex playerIndex) const
{
